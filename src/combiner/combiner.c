...
void combine_arrays(map_t terrain_data, double **kde_layout, double **cost_map) {
    int size_x = terrain_data.height,
        size_y = terrain_data.width,
    ...
    for (int x = 0; x < size_x; x++) {
        for (int y = 0; y < size_y; y++) { 
            int terrain_cost = lookup_table[terrain_data.data[x][y] - 1];  // Subtract one due to terrain_layout starting at 1
